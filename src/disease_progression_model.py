...
class DiseaseProgressionModel(object):
    ...
    def initialization(self):
        ...
        for i in range(len(self.__data)):
            ...
            for j in range(len(trajectory)):
                current_subtype = floor(random.uniform(0, self.__K - 0.00001))
                risk_tier = floor(random.uniform(0, self.__R - 0.00001))
        ...
        self.__r_count = np.sum(self.__n_r)
        self.__z_count = np.sum(self.__n_z)
        self.__o_count = np.sum(self.__n_o)
    ...
    def sampling_subtype(self, i, j):
        ...
        if j == 0:
            ...
            previous_subtype = self.__hidden_state_assignment[i][j-1][0]
            # in fact, the final state doesn't have descendent state
        ...
        if next_subtype == no_next_indicator:
            ...
            part_1_2_numerator = self.__alpha + self.__n_z[:, next_subtype]
    ...
    def log_likelihood(self, test_data=None):
        ...
        for patient_index in range(self.__M):
            ...
            for visit_index in range(self.__T[patient_index]):
                forward_cache[-1].append(list())

                initial_state = np.zeros([self.__K + 1])
                initial_state[self.__K] = 1
                forward_cache[-1].append(initial_state)
